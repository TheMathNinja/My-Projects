...
ADL25 <- mfl_connect(season = 2025, league_id = 60206, user_agent = "mikeyfili", rate_limit_number = 3, rate_limit_seconds = 6)
FAFL25 <- mfl_connect(season = 2025, league_id = 22686, user_agent = "mikeyfili", rate_limit_number = 3, rate_limit_seconds = 6)
...
get_league_players <- function(conn) {
  players <- ffscrapr::mfl_players(conn) %>%
    ...
    left_join(., y = ffscrapr::mfl_players() %>%
                rename(mfl_id = player_id,
                       mfl_default_pos = pos) %>%
                dplyr::select(c("mfl_id", "mfl_default_pos")),
              by = c("mfl_id")) %>%
    ...
    left_join(., y = nflreadr::load_ff_playerids() %>%
                dplyr::select(mfl_id, espn_id),
              by = "mfl_id") %>%
    mutate(espn_id = as.integer(coalesce(espn_id.x, espn_id.y))) %>%
    dplyr::select(-espn_id.x, -espn_id.y)  # Clean up temporary columns
  
  # Build df showing current league position changes. Should match MFL URL https://www46.myfantasyleague.com/2025/csetup?L=60206&C=CHGPOS
...
ADL_players_changes <- get_league_players(ADL25)
ADL_players_2025 <- ADL_players_changes$players_df
ADL_curr_pos_changes <- ADL_players_changes$curr_pos_changes_df

FAFL_players_changes <- get_league_players(FAFL25)
FAFL_players_2025 <- FAFL_players_changes$players_df
FAFL_curr_pos_changes <- FAFL_players_changes$curr_pos_changes_df


#Build df listing all current ESPN Fantasy positions.
espn_players_2025 <- ffscrapr::espn_players() %>%
...
ADL_players_2025_with_espn <- ADL_players_2025 %>%
  full_join(., y = espn_players_2025 %>%
              dplyr::select(c("espn_id",
                       "espn_player_name",
                       "espn_team",
                       "espn_pos")),
            by = c("espn_id")) %>%
...
MFL_espn_disagreements <- ADL_players_2025_with_espn %>%
  filter(mfl_default_pos != espn_pos)  %>%
  arrange(mfl_player_name)

view(MFL_espn_disagreements)

ADL_FAFL_disagreements <- ADL_players_2025 %>%
  rename(ADL_pos = league_pos) %>%
  left_join(., y = FAFL_players_2025 %>%
