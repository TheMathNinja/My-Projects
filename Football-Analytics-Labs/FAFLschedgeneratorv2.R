...
GMstats14 <- left_join(fullstandings14, GMnamekey14, by = "franchise_id")%>%
  select(owner_name, year, conference, franchise_name, allplay_wins, allplay_losses, allplay_ties, allplay_sum, allplay_winpct, points_for, pf_ratio, potential_points, pp_ratio, reg_adj_allplay_wins, reg_all_play_pct)
...
for(i in 15:19){
  ...
  assign(
    ...
    left_join(
      ...
      by = "franchise_id")%>%
      select(owner_name, year, conference, franchise_name, prev_regAPW, allplay_wins, allplay_losses, allplay_ties, allplay_sum, allplay_winpct, points_for, pf_ratio, potential_points, pp_ratio, reg_adj_allplay_wins, reg_all_play_pct)
  )
}

GMall = rbind(GMstats14, GMstats15, GMstats16, GMstats17, GMstats18, GMstats19)
view(GMall)
...
GMall <- GMall %>%
  mutate(returner = if_else(owner_name %in% GMall$owner_name[year == year-1], 1, 0))
